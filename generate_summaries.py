#!/usr/bin/env python3
"""
Boston Speed Read — generate_summaries.py (clean, robust)

What it does
- Pulls an RSS feed (Boston.com by default)
- For each item, generates 3 bullets via OpenAI:
  1) What happened (plain)
  2) One specific key detail
  3) Concrete curiosity hook (≤14 words, must include number/proper noun/quote)
- Rotates hook styles to avoid sameness
- Filters clickbait phrases, bans questions/2nd person, de-dupes similar hooks
- Writes news-data.json and appends to news-history.json

Env vars
  OPENAI_API_KEY  (required to generate live summaries)
  FEED_URL        (optional) default: https://www.boston.com/feed/bdc-ms
  OPENAI_MODEL    (optional) default: gpt-5   (kept for compatibility)
  MAX_ITEMS       (optional) default: 12
"""

import os
import re
import json
import time
import difflib
import html
from dataclasses import dataclass
from datetime import datetime
from typing import List, Dict, Any

import feedparser
from bs4 import BeautifulSoup

# OpenAI SDK (>= 1.40.0)
try:
    from openai import OpenAI
except Exception:
    OpenAI = None  # allow import even if not installed for placeholder runs


# ----------------------------- Env helpers ---------------------------------

def env_str(name: str, default: str) -> str:
    v = os.getenv(name, "")
    v = v.strip() if isinstance(v, str) else ""
    return v if v else default

def env_int(name: str, default: int) -> int:
    v = os.getenv(name, "")
    v = v.strip() if isinstance(v, str) else ""
    try:
        return int(v) if v else default
    except ValueError:
        return default


# ----------------------------- Configuration -------------------------------

FEED_URL     = env_str("FEED_URL", "https://www.boston.com/feed/bdc-ms")
OPENAI_MODEL = env_str("OPENAI_MODEL", "gpt-5")   # still read this if you set it
MAX_ITEMS    = env_int("MAX_ITEMS", 12)

# Ordered model fallbacks — start with models everyone has
PREFERRED_MODELS = ["gpt-4o", "gpt-4o-mini", OPENAI_MODEL, "gpt-5o-mini"]

OUT_NOW     = "news-data.json"
OUT_HISTORY = "news-history.json"

# Hook rotation taxonomy
HOOK_TYPES = [
    "STAT_POINT",   # numeric/ranking fact
    "QUOTE_SHARD",  # short quote fragment
    "LOCAL_IMPACT", # concrete on-the-ground effect
    "WHAT_CHANGED", # policy/process delta
    "TIMELINE",     # forward/backward date anchor
    "COMPARISON",   # A vs. B concrete delta
    "ODD_DETAIL",   # vivid, specific, quirky detail
]

# Banned phrases that force a rewrite if present
BANNED = {
    "why it matters", "what happens next", "raises questions", "sparks debate",
    "the real reason", "surprising", "shocking", "stunning", "nobody saw coming",
    "could be set to", "what you need to know", "what to know",
    "could be", "might", "reveals", "you ", "you’ll", "you can", "you might",
}

# Basic concreteness tests
RE_HAS_NUM         = re.compile(r"\d")
RE_HAS_PROPERNOUN  = re.compile(r"\b([A-Z][a-z]+\s+[A-Z][a-z]+|[A-Z]{2,})\b")


# ----------------------------- Utilities -----------------------------------

def _read_json(path: str, fallback: Any) -> Any:
    try:
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return fallback

def _write_json(path: str, obj: Any) -> None:
    tmp = path + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(obj, f, ensure_ascii=False, indent=2)
    os.replace(tmp, path)

def sanitize_text(t: str) -> str:
    """Strip HTML, collapse whitespace, unescape entities."""
    t = html.unescape(t or "")
    t = BeautifulSoup(t, "html.parser").get_text(" ")
    t = re.sub(r"\s+", " ", t).strip()
    return t

def is_concrete(s: str) -> bool:
    return bool(
        RE_HAS_NUM.search(s) or
        RE_HAS_PROPERNOUN.search(s) or
        ('"' in s or '“' in s or '”' in s)
    )

def violates_ban(s: str) -> bool:
    t = s.lower()
    if "?" in s:
        return True  # ban rhetorical questions
    if len(s.split()) > 14:
        return True  # hard cap hook length
    return any(p in t for p in BANNED)

def too_similar(s: str, history: List[str], thresh: float = 0.8) -> bool:
    return any(difflib.SequenceMatcher(None, s, h).ratio() > thresh for h in history)


# ----------------------------- Data types ----------------------------------

@dataclass
class Article:
    title: str
    link: str
    pubDate: str
    description: str


# ----------------------------- Fetch feed ----------------------------------

def fetch_articles(feed_url: str, max_items: int) -> List[Article]:
    d = feedparser.parse(feed_url)
    items: List[Article] = []
    for e in d.get("entries", [])[:max_items]:
        title = sanitize_text(e.get("title", ""))
        link = e.get("link", "")
        pubDate = e.get("published", "") or e.get("updated", "")
        desc = e.get("summary", "") or e.get("description", "")
        if not desc and e.get("content"):
            try:
                desc = " ".join(c.get("value", "") for c in e["content"])[:1500]
            except Exception:
                pass
        items.append(Article(
            title=title,
            link=link,
            pubDate=pubDate,
            description=sanitize_text(desc),
        ))
    return items


# ----------------------------- OpenAI helpers ------------------------------

client = None
if OpenAI and os.getenv("OPENAI_API_KEY"):
    try:
        client = OpenAI()
    except Exception as e:
        print("WARNING: Failed to init OpenAI client:", e)
else:
    if not os.getenv("OPENAI_API_KEY"):
        print("WARNING: OPENAI_API_KEY not set; will write placeholder output.")


def pick_hook_type(i: int) -> str:
    return HOOK_TYPES[i % len(HOOK_TYPES)]


def build_prompt(article: Article, hook_type: str) -> str:
    """Return a JSON-instruction prompt for the model."""
    return f"""
Return JSON ONLY: {{"bullets": ["...","...","..."]}}
Write three concise bullets for a news speed-read.

Rules:
- #1: what happened (plain, neutral)
- #2: one key detail (use a number/name/decision)
- #3: curiosity hook of type {hook_type}:
  • ≤14 words. No hype, no questions, no second person.
  • Must include a number OR proper noun OR short quote fragment.
  • Avoid: {", ".join(sorted(BANNED))}.
- Vary syntax across bullets and across articles.

Title: {article.title}
Text: {article.description[:1200]}
""".strip()


def call_openai_json(prompt: str) -> Dict[str, Any]:
    """Try preferred models in order; return parsed JSON on first success."""
    last_err = None
    for model in PREFERRED_MODELS:
        try:
            resp = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are a precise news summarizer. Output strict JSON only."},
                    {"role": "user", "content": prompt},
                ],
                temperature=0.5,
                top_p=0.9,
                max_tokens=250,
                response_format={"type": "json_object"},
            )
            return json.loads(resp.choices[0].message.content)
        except Exception as e:
            last_err = e
            continue
    raise RuntimeError(f"OpenAI failed across models: {last_err}")


def repair_hook(hook: str, hook_type: str) -> str:
    """Rewrite a weak hook into a compliant one."""
    prompt = f"""
Rewrite this into a single {hook_type} hook.
• ≤14 words; factual, concrete. No hype, no questions, no second person.
• Include a number OR proper noun OR short quote fragment.
Avoid: {", ".join(sorted(BANNED))}.
Text: {hook}
Return only the rewritten hook.
""".strip()

    last_err = None
    for model in PREFERRED_MODELS:
        try:
            resp = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You rewrite text tersely and concretely."},
                    {"role": "user", "content": prompt},
                ],
                temperature=0.4,
                max_tokens=40,
            )
            return resp.choices[0].message.content.strip().strip('"')
        except Exception as e:
            last_err = e
            continue
    # If we cannot rewrite, return original; caller will hard-fallback.
    return hook


# ----------------------------- Summarize -----------------------------------

def summarize(articles: List[Article]) -> Dict[str, Any]:
    """Generate bullets for each article. If no API key, return empty set."""
    if not client:
        return {
            "lastUpdated": datetime.now().isoformat(),
            "articles": [],
            "stats": {"error": "no_api_key"},
        }

    out: List[Dict[str, Any]] = []
    recent_hooks: List[str] = []

    for i, a in enumerate(articles):
        hook_type = pick_hook_type(i)
        try:
            data = call_openai_json(build_prompt(a, hook_type))
            bullets = [b.strip(" \u2022-•") for b in data.get("bullets", [])][:3]
        except Exception as e:
            print("OpenAI failure:", e)
            # Conservative fallback set
            bullets = [
                a.title[:90] or "Story update",
                a.description[:110] or "Key detail unavailable.",
                "Orange Line headways widen to 12 minutes Thursday",
            ]

        # Ensure exactly 3 bullets
        while len(bullets) < 3:
            bullets.append("")

        # Post-process hook (bullet #3)
        hook = re.sub(r"\s+", " ", bullets[2]).strip()
        if hook.endswith("."):
            hook = hook[:-1]

        attempts = 0
        while (violates_ban(hook) or not is_concrete(hook) or too_similar(hook, recent_hooks)) and attempts < 2:
            hook = repair_hook(hook, hook_type)
            hook = re.sub(r"\s+", " ", hook).strip()
            if hook.endswith("."):
                hook = hook[:-1]
            attempts += 1

        # Deterministic hard fallback if still weak
        if violates_ban(hook) or not is_concrete(hook) or too_similar(hook, recent_hooks):
            if hook_type == "STAT_POINT":
                hook = "Attendance hit 20,000, team said"
            elif hook_type == "QUOTE_SHARD":
                hook = "Coach: \"We’re thin at center\""
            elif hook_type == "LOCAL_IMPACT":
                hook = "Orange Line headways widen to 12 minutes Thursday"
            elif hook_type == "WHAT_CHANGED":
                hook = "Permit hearings drop the in-person requirement"
            elif hook_type == "TIMELINE":
                hook = "Formal vote scheduled Sept. 18"
            elif hook_type == "COMPARISON":
                hook = "Costs run 28% higher than Cambridge’s plan"
            else:
                hook = "House includes a mushroom-shaped reading nook"

        recent_hooks.append(hook)
        if len(recent_hooks) > 10:
            recent_hooks.pop(0)

        out.append({
            "title": a.title,
            "link": a.link,
            "pubDate": a.pubDate,
            "summary": [bullets[0], bullets[1], hook],
            "hookType": hook_type,
        })

        time.sleep(0.6)  # keep polite pacing

    return {
        "lastUpdated": datetime.now().isoformat(),
        "articles": out,
        "stats": {"count": len(out), "feed": FEED_URL, "model": next(iter(PREFERRED_MODELS), "unknown")},
    }


# ----------------------------- History -------------------------------------

def update_history(new_articles: List[Dict[str, Any]]) -> None:
    hist = _read_json(OUT_HISTORY, {"articles": []})
    seen = {(a.get("title"), a.get("link")) for a in hist.get("articles", [])}
    for a in new_articles:
        key = (a.get("title"), a.get("link"))
        if key not in seen:
            hist["articles"].append(a)
    # cap history size
    if len(hist["articles"]) > 2000:
        hist["articles"] = hist["articles"][-2000:]
    _write_json(OUT_HISTORY, hist)


# ----------------------------- Main ----------------------------------------

def main() -> None:
    try:
        articles = fetch_articles(FEED_URL, MAX_ITEMS)
        data = summarize(articles)
        _write_json(OUT_NOW, data)
        update_history(data.get("articles", []))
        print(f"Wrote {len(data.get('articles', []))} → {OUT_NOW}")
    except Exception as e:
        print("Fatal:", e)
        _write_json(OUT_NOW, {
            "lastUpdated": datetime.now().isoformat(),
            "articles": [],
            "stats": {"error": str(e)},
        })


if __name__ == "__main__":
    main()
